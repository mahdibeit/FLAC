...
def load_data(seed, IID):
    ...
    trainset = MNIST('Clients/data/mnist/', train=True, download=True, transform=trans_mnist)
    testset = MNIST('Clients/data/mnist/', train=False, download=True, transform=trans_mnist)
    if IID==True:
...
def main():
    ...
    # Start client
    fl.client.start_numpy_client("localhost:8080", client=MnistClient(args))
